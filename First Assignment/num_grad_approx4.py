...
def f(x, y):
    return np.log(1+(1/2)*(x**3+3*y**2))

def grad_x(x,y):
    return (3*x**2)/(2+x**3+3*y**2)

def grad_y(x,y):
    return (6*y)/(2+x**3+3*y**2)
...
for i in range(0,3):
    ...
    print("Gradient analytically computed: \ngrad_x = "+ str(out_grad_x) + ", " + "grad_y = " + str(out_grad_y))
    print("Numerical approximation of gradient: \ngrad_x = "+ str(out_grad_approx_x) + ", " + "grad_y = " + str(out_grad_approx_y) + "\n")
